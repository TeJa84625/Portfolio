...
function renderProjectCards(projects) {
    ...
    projects.forEach(project => {
        ...
        // Determine the label for the second stat (downloads/visits)
        const statTwoLabel = project.button_label && project.button_label !== "Download"
                               ? project.button_label.replace('Visit ', '').replace('View ', '') // Clean up label for brevity
                               : "Downloads"; // Default to Downloads

        const statTwoValue = project.button_label === "Download"
                               ? (project.downloads !== undefined ? project.downloads : 0)
                               : (project.visits !== undefined ? project.visits : 0); // Assuming 'visits' for other button types
